...
export default function RecordScreen() {
    ...
    try {
      ...
      if (uri && sessionId && sessionDirectory.current) {
        ...
        try {
          ...
          await audioService.uploadFinalChunk(
            finalChunkUri,
            sessionId,
            chunkNumber.current
          );
  ...
  const handleBluetoothPress = async () => {
    try {
      ...
      if (Platform.OS === "android") {
        const bluetoothPermission = await check(
      ...
      // Parar o scan após 5 segundos
      setTimeout(() => {
        bleManager.stopDeviceScan();
        setIsScanning(false);
      }, 5000);
    } catch (error) {
      console.error("Erro ao acessar BLE:", error);
      Alert.alert("Erro", "Não foi possível acessar o Bluetooth");
      setIsScanning(false);
    }
  };
  ...
  return (
    <View style={styles.container}>
        ...
        {isCheckingConnection ? (
          ...
          <TouchableOpacity onPress={checkApiConnection}>
            <Text style={styles.errorText}>
              Erro de conexão. Toque para tentar novamente.
            </Text>
          </TouchableOpacity>
        ) : isConnected ? (
      ...
      >
        ...
        >
          ...
          >
            ...
            {isScanning && <Text>Procurando dispositivos...</Text>}
            ...
            <ScrollView style={{ maxHeight: 200 }}>
              {discoveredDevices.map((device, idx) => (
                ...
                >
                  <Text>
                    {device.name || "Dispositivo Desconhecido"} ({device.id})
                  </Text>
                </TouchableOpacity>
              ))}
            </ScrollView>
      ...
      >
        <BluetoothLogo width={30} height={30} />
      </TouchableOpacity>
      <Nav />
    </View>
  );
